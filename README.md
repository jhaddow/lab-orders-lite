...
- **Dev-mode auth, production-shaped helper.** `getCurrentUser()` reads a plain user-id cookie (httpOnly + sameSite=lax; no signing in dev mode); `/sign-in-as` is a no-password user picker. Real auth would slot in behind the same helper with no call-site changes. Role gating happens at the repo boundary (`requireRole(actor, "ADMIN")`), not in the UI; the UI hides admin-only affordances as UX polish.
